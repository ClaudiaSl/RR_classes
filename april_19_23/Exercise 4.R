# install.packages('styler') #To have add-ins styling code
rm(list=ls())
# Sets the path to the parent directory of RR classes
setwd("C:\\Users\\cslab\\Desktop\\sem 6\\RR\\RR_classes\\april_19_23")
...
# This will calculate total workers in every country and produce vector for every country.
# For loop takes into consideration columns from 3:11 which are countries columns

for (i in 3:11){
  vectorx<- isco1[,i] + isco2[,i] + isco3[,i] + isco4[,i] + isco5[,i] + isco6[,i] + isco7[,i] + isco8[,i] + isco9[,i]
  # Data frame is converted into data.table to easily convert it into vector
  vectorx <- data.table(vectorx)
  vec <- melt.data.table(vectorx[1:40], measure.vars = 1)[, value]
  assign(paste("total_",colnames(isco1[i]),sep=""),vec)
  rm(vectorx, vec)
}
